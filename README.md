...
验收脚本会真实创建来源、材料、批次、项目、需求、消耗和颜色变化，验证余额从 1000g 降到 500g、撤销后恢复 1000g、最新流水为 `REVERSAL`、颜色和搜索正确；随后验证调整复核链：阈值内调整直接入账、超限调整暂存不动余额、同会话复核被拒、第二会话批准后余额恰好变动一次、重复批准返回 409 且余额不变、拒绝后余额不变。
...
- 消耗、调整和撤销都在事务中锁定批次。
- 批次余额绝不允许小于 0。
- 材料可设置调整复核阈值，超限调整先暂存为待复核请求，不改动余额。
- 复核人必须在另一会话中输入操作员密码，提交会话不能复核自己的调整。
- 批准与库存流水写入在同一事务完成，一条复核请求最多产生一条流水，重复批准不会重复增减余额。
